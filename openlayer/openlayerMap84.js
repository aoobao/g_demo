(function (window, ol) {
	function OpenlayerMap(options) {
		...
		// 		市级：市级：http://ditu.zj.cn:8089/geoserver/zjplatform/D380025/wms?request=GetCapabilities
		// 区县级
		// 区县级：县级：http://ditu.zj.cn:8089/geoserver/zjplatform/D380027/wms?request=GetCapabilities
		// 乡镇级
		// 乡镇级： 级： http://ditu.zj.cn:8089/geoserver/zjplatform/D380033/wms?request=GetCapabilities

		var wgSource1 = new ol.source.TileWMS({
		...
		var wgSource2 = new ol.source.TileWMS({
			// url: 'http://ditu.zj.cn:8089/geoserver/zjplatform/D380033/wms',
			url: 'http://ditu.zj.cn:8089/geoserver/zjplatform/D380027/wms',
			projection: projection,
			tileGrid: new ol.tilegrid.WMTS({
				origin: ol.extent.getTopLeft(projectionExtent),
				resolutions: resolutions,
			...
			params: {
				'LAYERS': 'zjplatform:D380027',
				'VERSION': '1.1.0'
			}
		});

		var wgSource3 = new ol.source.TileWMS({
			// url: 'http://ditu.zj.cn:8089/geoserver/zjplatform/D380033/wms',
			url: 'http://ditu.zj.cn:8089/geoserver/zjplatform/D380033/wms',
			projection: projection,
			tileGrid: new ol.tilegrid.WMTS({
		...
		self.map = new ol.Map({
			...
			view: new ol.View({
				...
				//				}),
				//center: [120.49038, 30.08370],
				//center:[120.482406383648, 30.0002705746275],
				center: [120.18892442609743, 29.16634946521879], //嘉兴.	
				zoom: 8,
				minZoom: 5,
				maxZoom: 20
			})
		...
		self.map.getView().on("change:resolution", function () {
			...
			var l3 = self.map.getLayers().getArray()[2];
			if (zoom < 10) {
				l3.setSource(wgSource1)
			} else if (zoom < 14) {
				l3.setSource(wgSource2)
			} else {
				l3.setSource(wgSource3)
			}
			if (zoom <= 14) {
